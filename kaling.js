...
module.exports = {
    login: function (email, password, apiKey, url) {
        ...
        if(!/^http(s)?\:\/\/.+/.test(location)) throw new ReferenceError("url 형식이 아니에요 :(");
        ...
        switch (ls) {
            case 200:
                ...
                let tiara = function () {
                    Jsoup.connect('https://track.tiara.kakao.com/queen/footsteps').ignoreContentType(true).execute().cookie('TIARA')
                }
